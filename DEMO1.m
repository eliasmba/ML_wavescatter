...
x = [+1.1*ones(1,size(Y,2)); linspace(-0.7,0.7,size(Y,2));0.5*ones(1, size(U,2))];
...
Sigma = @(y)repmat(coeff_a.*u_mode(y, rhob, rho0, omega),1,4);
...
C = capacitance(y,R,rho0, rhob, kappa0, kappab, delta);
